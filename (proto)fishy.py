        ...
        while True:
            ...
            if centers is None:
                ...
                if time.time() - start_time >= 7:
                    print("Время попыток истекло. Повторяем поиск...")
                    return  # Выходим из функции, повторяя поиск объекта 3
                continue  # Если объекты не найдены, продолжаем цикл
            
            obj1, obj2 = centers
            print(f"Координаты объекта 1: {obj1}, Координаты объекта 2: {obj2}")

            # Определение расположения объектов
...
threshold3_value = 0.5  # Установите уровень доверия для соответствия третьего шаблона
